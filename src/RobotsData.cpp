...
{
    /// enemy 1
    if (comp[0] && gladiator->game->getOtherRobotData(comp[0]).lifes > 0 && sqrt(sq(gladiator->game->getOtherRobotData(comp[0]).position.x - gladiator->robot->getData().position.x) + sq(gladiator->game->getOtherRobotData(comp[0]).position.y - gladiator->robot->getData().position.y)) < range)
    {
    ...
    else if (comp[1] && gladiator->game->getOtherRobotData(comp[1]).lifes > 0 && sqrt(sq(gladiator->game->getOtherRobotData(comp[1]).position.x - gladiator->robot->getData().position.x) + sq(gladiator->game->getOtherRobotData(comp[1]).position.y - gladiator->robot->getData().position.y))  < range)
    {
